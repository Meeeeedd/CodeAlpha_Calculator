...
const translations = {
  en: {
    ...
    aiPlaceholder:
      "e.g., A shirt costs $25 with a 20% discount. What is the final price?",
  ...
  fr: {
    ...
    aiModalDescription:
      "Écrivez un problème mathématique et laissez l'IA le résoudre.",
    aiPlaceholder:
      "Ex: Une chemise coûte 25€ avec 20% de réduction. Quel est le prix final ?",
  ...
  es: {
    ...
    aiModalDescription:
      "Escribe un problema matemático y deja que la IA lo resuelva.",
    aiPlaceholder:
      "Ej: Una camisa cuesta $25 con un 20% de descuento. ¿Cuál es el precio final?",
...
class Calculator {
  ...
  appendNumber(number) {
    ...
    if (this.displayNeedsReset) this.currentOperand = "";
    this.displayNeedsReset = false;
    this.currentOperand =
  ...
  calculate() {
    ...
    if (this.operation === "÷" && current === 0) {
      this.showError(translations[currentLanguage].errorDivideByZero);
      return false;
    }
    let result;
    const expression = `${this.getFormattedNumber(this.previousOperand)} ${
      this.operation
    } ${this.getFormattedNumber(this.currentOperand)}`;
  ...
  updateHistoryDisplay() {
    ...
    if (this.history.length === 0) {
      ...
      this.history.forEach((item, index) => {
        ...
        li.innerHTML = `<span class="expression">${
          item.expression
        } =</span><span class="result">${this.getFormattedNumber(
          item.result
        )}</span>`;
  ...
  getFormattedNumber(numberString) {
    ...
    return (
      (isNaN(parseFloat(int)) ? "" : parseFloat(int).toLocaleString(locale)) +
      (dec != null ? `.${dec}` : "")
    );
  }
  updateDisplay() {
    this.currentOperandElement.innerText = this.getFormattedNumber(
      this.currentOperand
    );
  ...
  button.addEventListener("click", () => {
    ...
    if (number) {
      ...
      calculator.chooseOperation(
        { divide: "÷", multiply: "×", subtract: "−", add: "+" }[operator]
      );
    } else if (action) {
      switch (action) {
        case "clear":
    ...
    if (shouldUpdate) calculator.updateDisplay();
  });
});
...
solveProblemBtn.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch("/api/solve", {
      ...
      body: JSON.stringify({
        problem: wordProblemInput.value,
        language: currentLanguage,
      }),
    });
    if (!response.ok) throw new Error(`Server error: ${response.statusText}`);
    const data = await response.json();
    aiExpression.textContent = data.expression || "Not found";
    aiAnswer.textContent = data.answer || "Not found";
  } catch (error) {
